...
class ErosBrowserControls extends HTMLElement {
    ...
    renderTabs() {
        const tabs = ["depth", "canny", "lineart", "openpose", "scribble", "softedge", "normal", "segmentation", "original"];
        ...
        tabs.forEach(tab => {
            ...
            el.innerText = tab;
            el.onclick = () => {
                this.shadowRoot.querySelectorAll('.eros-tab').forEach(t => t.classList.remove('active'));
    ...
    renderBlendModes(modesStr) {
        const modes = (modesStr || "normal,multiply,screen,overlay,luminosity,difference").split(',');
    ...
    setupEvents() {
        const root = this.shadowRoot;
        const emit = (name, val) => this.dispatchEvent(new CustomEvent(name, { detail: val }));
        ...
        root.getElementById('chk-overlay').onchange = (e) => emit('setting-change', { key: 'overlayEnabled', value: e.target.checked });
        root.getElementById('chk-badges').onchange = (e) => emit('setting-change', { key: 'showTagBadges', value: e.target.checked });
        root.getElementById('chk-cache').onchange = (e) => emit('setting-change', { key: 'cacheBusting', value: e.target.checked });
        root.getElementById('sel-blend').onchange = (e) => emit('setting-change', { key: 'blendMode', value: e.target.value });
        root.getElementById('rng-opacity').oninput = (e) => emitDebounced('setting-change', { key: 'opacity', value: parseFloat(e.target.value) });
        root.getElementById('rng-columns').oninput = (e) => emitDebounced('setting-change', { key: 'columns', value: parseInt(e.target.value) });
        root.getElementById('rng-badgesize').oninput = (e) => emitDebounced('setting-change', { key: 'badgeSize', value: parseInt(e.target.value) });
        root.getElementById('btn-refresh').onclick = () => emit('refresh-requested');
    }
}
...
class ErosImageGrid extends HTMLElement {
    ...
    set config(cfg) {
        ...
        if (cfg.badgeSize) this.grid.style.setProperty('--badge-font-size', cfg.badgeSize + 'px');
        this.cfg = cfg; // store for rendering
        if (this.lastFiles) this.render(this.lastFiles, this.lastImgTags, this.lastCachePath, this.lastTab, this.lastSelected);
    }

    render(files, imageTags, cachePath, currentTab) {
        this.lastFiles = files; this.lastImgTags = imageTags; this.lastCachePath = cachePath; this.lastTab = currentTab;
        if (!this.grid) return;
        ...
        files.forEach(f => {
            ...
            const imgPath = `/eros/cache/view_image?path=${encodeURIComponent(cachePath)}&subfolder=${currentTab}&filename=${encodeURIComponent(f)}${ts}`;
            ...
            if (this.cfg?.overlayEnabled) {
                const ovPath = `/eros/cache/view_image?path=${encodeURIComponent(cachePath)}&subfolder=original&filename=${encodeURIComponent(f)}${ts}`;
            ...
            const tags = imageTags.get(f.split('/').pop().replace(/\.[^/.]+$/, ""));
            if (this.cfg?.showTagBadges && tags && tags.size > 0) {
                html += `<div class="eros-tag-badges">`;
                Array.from(tags).slice(0, 6).forEach(t => html += `<span class="eros-tag-badge">${t}</span>`);
                if (tags.size > 6) html += `<span class="eros-tag-badge" style="background:#555;">+${tags.size - 6}</span>`;
                html += `</div>`;
            }
            item.innerHTML = html + `<div class="eros-item-label">${f.split('/').pop().replace(/\.[^/.]+$/, "")}</div>`;

            item.onclick = () => {
                this.shadowRoot.querySelectorAll('.eros-item').forEach(i => i.classList.remove('selected'));
                item.classList.add('selected');
                this.dispatchEvent(new CustomEvent('image-selected', { detail: { filename: f, imgPath } }));
            };
...
class ErosTagSidebar extends HTMLElement {
    constructor() { super(); this.attachShadow({ mode: 'open' }); this.collapsed = { filter: false, selected: false }; }
    ...
    setupEvents() {
        ...
        root.getElementById('btn-toggle-filter').onclick = () => {
            ...
            root.getElementById('filter-content').style.display = this.collapsed.filter ? 'none' : 'flex';
        ...
        root.getElementById('btn-toggle-selected').onclick = () => {
            ...
            root.getElementById('selected-content').style.display = this.collapsed.selected ? 'none' : 'flex';
        ...
        root.getElementById('inp-new-tag').onkeydown = (e) => { if (e.key === 'Enter') addTag(); };
        root.getElementById('btn-add-tag').onclick = addTag;
        root.getElementById('btn-auto-tag').onclick = () => this.dispatchEvent(new CustomEvent('tag-auto'));
    }
    ...
    renderFilters(allTags, activeFilters, query) {
        ...
        allTags.forEach((count, name) => {
            if (!name || (query && !name.toLowerCase().includes(query.toLowerCase()))) return;
            const chip = document.createElement('div');
            chip.className = `eros-tag-chip ${activeFilters.has(name) ? 'active' : ''}`;
            chip.innerHTML = `${name} <span style="opacity:0.6;font-size:9px;">${count}</span>`;
            chip.onclick = (e) => this.dispatchEvent(new CustomEvent('filter-click', { detail: { name, ctrlKey: e.ctrlKey } }));
    ...
    renderSelected(tags, hasSelection) {
        ...
        if (!hasSelection) {
            chips.innerHTML = '<div style="font-size:10px; color:#666;">Select an image to manage tags</div>';
        ...
        if (!tags || tags.size === 0) {
            chips.innerHTML = '<div style="font-size:10px; color:#888; font-style:italic;">No tags</div>';
        } else {
            tags.forEach(t => {
                const chip = document.createElement('div');
...
class ErosCacheBrowser extends HTMLElement {
    constructor() {
        ...
        let stored = {};
        try { stored = JSON.parse(localStorage.getItem('eros_cache_browser_settings') || "{}"); }
    ...
    saveSettings() {
        try {
            localStorage.setItem('eros_cache_browser_settings', JSON.stringify(this.settings));
        } catch (e) {
    ...
    connectedCallback() {
        ...
        window.addEventListener('keydown', (e) => { if (e.key === 'Escape' && this.state.isOpen) this.close(); });
    }
    ...
    setupOrchestration() {
        ...
        sidebar.addEventListener('tag-add', (e) => this.cache.addTag(this.getBasename(this.state.currentFilename), e.detail));
        sidebar.addEventListener('tag-remove', (e) => this.cache.removeTag(this.getBasename(this.state.currentFilename), e.detail));
        sidebar.addEventListener('tag-auto', () => this.cache.autoTag(this.getBasename(this.state.currentFilename)));
        ...
        sidebar.addEventListener('filter-click', (e) => {
            ...
            if (ctrlKey) {
                ...
                if (this.state.activeFilters.has(name)) this.state.activeFilters.delete(name);
        ...
        grid.addEventListener('image-selected', (e) => {
            ...
            // Node Update Logic
            if (this.activeNode) { // Fixed: removed .widgets check, safer below
    ...
    getBasename(filename) { return filename ? filename.split('/').pop().replace(/\.[^/.]+$/, "") : ""; }
    ...
    renderGrid() {
        ...
        if (query || this.state.activeFilters.size > 0) {
            files = files.filter(f => {
                ...
                if (query) {
                    ...
                    if (!match && tags) {
                        for (let t of tags) if (t.toLowerCase().includes(query)) { match = true; break; }
                    }
                ...
                if (this.state.activeFilters.size > 0) {
                    ...
                    for (let ft of this.state.activeFilters) if (!tags.has(ft)) return false;
                }
                return true;
            });
        }
        ...
        // Pass to Grid
        this.els.grid.render(files, this.cache.imageTags, this.cache.cachePath, this.state.currentTab, this.state.currentFilename);
        this.updateSidebarFilters();
    }

    // Helpers to bridge Grid/State -> Sidebar
    pushGridData() {
        this.els.grid.render(this.state.files, this.cache.imageTags, this.cache.cachePath, this.state.currentTab, this.state.currentFilename);
        this.renderGrid();
    }

    updateSidebarFilters() {
        this.els.sidebar.renderFilters(this.cache.allTags, this.state.activeFilters, this.state.tagSearchQuery);
    }

    updateSidebarSelection() {
        const base = this.getBasename(this.state.currentFilename);
        const tags = this.cache.imageTags.get(base);
        this.els.sidebar.renderSelected(tags || new Set(), !!this.state.currentFilename);
    }
}
